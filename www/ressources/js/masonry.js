...
(function($, window, document, undefined) {
    ...
    var Portfolio = {
            ...
            for (var i = 0; i <= thumbnails.length; i++) {
                ...
                if (href == $(thumbnails[i]).attr('href')) {
                    ...
                    if (i !== (thumbnails.length - 1)) {
                        ...
                        if ($(window).width() <= 1200 && $(window).width() > 800) cols = 3;
                        else if ($(window).width() <= 800 && $(window).width() > 540) cols = 2;
                        else if ($(window).width() <= 540) cols = 1;
        ...
        showContent: function(thumbnail) {
            ...
            self.$elem.find('li a.thumbnail').removeClass('active');
            ...
            var html = '<li class="content"><i class="fa fa-times-circle fa-2x close"></i>' + $portfolioContent.html() + '</li>';
            ...
            $('html, body').animate({
                //scrollTop: self.$elem.find('li.content').offset().top -265
            }, 700);

            // Close content
            self.$elem.find('.close').click(function(){ 
                self.$elem.find('li a.thumbnail').removeClass('active');
                self.$elem.find('li a.thumbnail .active-arrow').fadeOut("normal", function() { $(this).remove(); } );
                self.$elem.find('li.content').fadeOut("normal", function() { $(this).remove(); } );
            });
        }
    };
