...
class quick extends StormComponent
{
	...
	public function backup()
	{
		...
		try
		{
			$this->log_model->Log($logfile, "Sauvegarde rapide...");
			$result = $this->backup_model->quickBackup($data);
			$this->log_model->Log($logfile, "Réussite");
		...
		{
			$this->log_model->Log($logfile, "Echec. Raison : " . $e->__toString());
	...
	public function download()
	{
		...
		header('Content-Disposition: attachment; filename="quick backup '. strftime("%Y-%m-%d %H-%M") .'.zip"');
