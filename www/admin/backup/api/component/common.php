...
class common extends StormComponent
{
	...
	public function checkInstall()
	{
		...
		if (!class_exists('ZipArchive'))
			$errors[] = array('type' => 'error', 'title' => 'Error!', 'message' => 'The ZipArchive php extension is disabled. This means that the system cannot operate with (create) zip files. This will cause all backups to fail!');
		...
		if (count($backups) == 0)
		{
			$errors[] = array('type' => 'warning', 'title' => 'Routine Cron', 'message' => 'Merci de paramétrer une routine Cron pour que les sauvegardes automatiques puissent s\'effectuer :'."\n\n".'php '.  realpath(rtrim(Storm::ToAbsolute('../cron.php'))) . ' > /dev/null 2>&1');
		}
		...
		foreach ($backups as $backup)
		{
			if (isset($backup->errors))
			{
				...
				{
					$errors[] = array('type' => 'error', 'title' => '`'. $backup->title .'` a échoué', 'message' => 'Erreur : '. $error->message);
				}
			...
			if (isset($backup->warnings))
			{
				...
				{
					$errors[] = array('type' => 'warning', 'title' => '`'. $backup->title .'` avertissements', 'message' => $error->message);
				}
	...
	private function getTree($root, $dir, $password, $type, $urlParams = '')
	{
		...
		foreach ($dir->GetDirs() as $subdir)
		{
			...
			$obj->childrenUrl = '/admin/backup/api/index.php?path=/getDirTree&key='. rawurlencode($password) .'&type='.
	...
	public function getDirTree($key, $root = null, $type = 'local', $sftpKey = '')
	{
		...
		if ($root == null) {
			$root = LocalDirectory::GetTopReadableParent(dirname(__FILE__));
		}
		...
		if (!empty($sftpKey)) {
			...
			if ($sftpKey_data == '')
			{
				throw new Exception('Invalid key name @common::getDirTree');
			}
		}
	...
	public function getDatabaseTree($host, $user, $password, $port = 3306)
	{
		...
		$databases = array('0' => 'xxxxxxxxxxxxxxxxxxx');
