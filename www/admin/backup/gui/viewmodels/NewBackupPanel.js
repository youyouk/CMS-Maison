function NewBackupPanel(data) {
	...
	this.refreshSFTPKeys = function (firstTime) {
		...
		$.getJSON('backup/api/index.php?path=/privatekeys/getKeys&key=' + Application.password(), function (resultData) {
	...
	this.refreshDropbox = function () {
		...
		$.getJSON('backup/api/index.php?path=/dropbox/getAuthorizedAccounts&key=' + Application.password(), function (data) {
	...
	this.newDropbox = function () {
		...
		$.getJSON('backup/api/index.php?path=/dropbox/getAuthorizeUrl&key=' + Application.password(), function (data) {
	...
	this.getFTPTree = function (included, excluded) {
		var treeview = new TreeView({
			url: 'backup/api/index.php?path=/admin/backup/getDirTree&type=' + self.destType() + '&root=' + encodeURIComponent(self.ftpUser() + (self.ftpPassword() ? ':' + self.ftpPassword() : '') + '@' + self.ftpHost().replace(/^s?ftp\:\/\//, '') + '/') + (self.sftpUsePrivateKey() ? '&sftpKey=' + self.sftpPrivateKey() : '') + '&key=' + Application.password(),
			selectable: true,
			dirSelectMode: true,
			singleSelection: true,
			disabledNonReadable: true,
			...
			dataError: function (target, isRoot) {
				...
				if (isRoot)
					self.ftpError("Erreur de connexion au FTP");
				else
					self.ftpError("Erreur de récupération des données");
			}
		});
	...
	this.submit = function() {
		...
		$.post('backup/api/index.php?path=/backups/add&overwrite=' + (self.isEdit() ? 'true' : 'false') + '&key=' + Application.password(),
		{
			title: self.title(),
			...
			if (self.isEdit())
				Application.alert('OK', "Sauvegarde mise à jour",'success' );
			else
				Application.alert('OK', "Sauvegarde ajoutée", 'success');

			Application.hideNewBackupPanel();
		}).error(function (xhr) {
			if (xhr.status == 409)
				Application.alert('Echec', "Ce nom de sauvegarde existe déjà", 'error');
			else if (xhr.status == 400)
				Application.alert('Echec', "Certains champs requis ne sont pas remplis", 'error');
			else
